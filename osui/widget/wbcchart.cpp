...
WBCChart::WBCChart(QWidget *parent):QWidget(parent)
{
...
void WBCChart::paintEvent(QPaintEvent *event)
{
    //绘制准备工作,启用反锯齿
    ...
    drawTable(&painter);
    //qDebug()<<QString("%1").arg(QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss"));
    if(this->data.count() > 0)
    {
        ...
        QPen pen2;
        pen2.setWidth(1);
        pen2.setColor(QColor(0,0,255));//直方图颜色
        painter.setPen(pen2);
...
void WBCChart::drawBackground(QPainter *painter)
{
    painter->save();
...
void WBCChart::drawTable(QPainter *painter)
{
...
void WBCChart::drawImage(QPainter *painter)
{

}

void WBCChart::resetData(QVector<int> &statisticsWBC)
{
    //qDebug()<<statisticsWBC.count();
    this->data = statisticsWBC;
    update();
}
