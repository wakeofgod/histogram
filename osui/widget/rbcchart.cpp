...
RBCChart::RBCChart(QWidget *parent):QWidget(parent)
{
...
void RBCChart::resetData(QVector<int> &statisticsRBC)
{
    this->data = statisticsRBC;
    update();
}

void RBCChart::paintEvent(QPaintEvent *event)
{
    //绘制准备工作,启用反锯齿
    ...
    if(this->data.count() > 0)
    {
        ...
        QPen pen2;
        pen2.setWidth(1);
        pen2.setColor(QColor(255,0,0));//直方图颜色
        painter.setPen(pen2);
...
void RBCChart::drawBackground(QPainter *painter)
{
    painter->save();
...
void RBCChart::drawTable(QPainter *painter)
{
...
void RBCChart::drawImage(QPainter *painter)
{

}

void RBCChart::drawRBCStaticImage(QPainter *painter, int model)
{
